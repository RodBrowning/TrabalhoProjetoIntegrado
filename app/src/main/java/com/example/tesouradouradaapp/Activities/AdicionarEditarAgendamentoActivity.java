...
public class AdicionarEditarAgendamentoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTituloParaActivity(intent, VizualizarAgendamentoActivity.EDITAR);

        if (intent.hasExtra(VizualizarAgendamentoActivity.EDITAR)) {
            AgendaViewModel agendaViewModel = new AgendaViewModel(getApplication());
            try {
                agendamentoParaEditar = agendaViewModel.getAgendamento(intent.getIntExtra(VizualizarAgendamentoActivity.ID_AGEDAMENTO_EDITAR, 0));
